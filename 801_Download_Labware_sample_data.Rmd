...
ALSO NOTE:  
* This script needs a bit of change every year - specifically, one needs to copy one of the parts 4-7 and change year in it  
* Also remember to change 20  
...
* Not needed if credentials have been set using keyring::key_set  
    - See ?get_nivabase_selection for walk-through 
...
# Get projects, but doesn't really make sense since the projects names differ from those in PROJECT in table LABWARE_CHECK_SAMPLE
if (FALSE){
...
prosj <- c('O 19330;ANA19 - MILKYS2019',
           'O 210330;ANA21 - MILKYS 2021; Hovedanalyser 2021')

prosj <- c('O 210330','ANA21 - MILKYS 2021; Hovedanalyser 2021')

labware_2020 <- get_nivabase_selection(
  "*",
  "LABWARE_CHECK_SAMPLE", 
  "TEXT_ID",
  dat_2020$TEXT_ID, values_are_text = TRUE)

labware_2020 <- get_nivabase_data(
  "select * from NIVADATABASE.LABWARE_CHECK_SAMPLE where PROSJEKT in 'prosj','") %>%
  mutate(PROSJEKT = oracle_fix_norw_letters(PROSJEKT),
         DESCRIPTION = oracle_fix_norw_letters(DESCRIPTION)
         )
...
check3 <- get_nivabase_data(
  "select * from NIVADATABASE.LABWARE_CHECK_SAMPLE where CUSTOMER in 'MSC'")
table(check3$PROSJEKT)
check3 %>%
  count(ACCOUNT_NUMBER, PROSJEKT, Year = year(SAMPLED_DATE)) %>%
  filter(Year %in% find_year)


```


### b1. Get 'check sample' data    
```{r}
...
prosj <- "O 210200.ANAIN"
cat("Name of project:", prosj, " \n\n")

labware_2021 <- get_nivabase_selection(
  "*",
  "LABWARE_CHECK_SAMPLE",
  "PROSJEKT",
  prosj, values_are_text = TRUE
  )

# Should be mostly 2021! 
check_dates <- labware_2021$SAMPLED_DATE
table(year(check_dates))
...
labware_import_2021 <- get_nivabase_selection(
  "*",
  "LABWARE_IMPORT",
  "PROSJEKT",
  prosj, values_are_text = TRUE
...
# Should be mostly 2021! 
check_dates <- labware_import_2021$SAMPLED_DATE
table(year(check_dates))
...
# o # o # o # o # o # o # o # o # o # o 
# NOTE: HARD-CODED project name  
# o # o # o # o # o # o # o # o # o # o 

prosj <- "O 210200.ANAIN"
cat("Name of project:", prosj, " \n\n")

labware_2021plus <- get_nivabase_selection(
  "*",
  "LABWARE_CHECK_SAMPLE",
  "PROSJEKT",
  prosj, values_are_text = TRUE
...
check_dates1 <- labware_2021plus$SAMPLED_DATE
table(addNA(year(check_dates1)))

check_dates2 <- substr(labware_2021plus$TEXT_ID, 4, 7)
table(check_dates2)

# Check those with conflicting year info  
df_check <- labware_2021plus %>% 
  filter(!year(SAMPLED_DATE) %in% 2022 & substr(labware_2022$TEXT_ID, 4, 7) == "2022")

labware_2022 <- labware_2021plus %>% 
  filter(SAMPLED_DATE >= ymd_hms("2022-03-01 00:00:00"),
         SAMPLED_DATE < ymd_hms("2023-03-01 00:00:00")) 
nrow(labware_2022)
...
if (FALSE){
  ...
  filename <- paste0("Labware_samples_2021_", file_date, ".rds")
  saveRDS(labware_2022, paste0("Files_to_Jupyterhub_2021/", filename))
  cat("File", sQuote(filename), "saved for 2021 \n")
  
  filename <- paste0("Labware_import_2021_", file_date, ".rds")
  saveRDS(labware_import_2021, paste0("Files_to_Jupyterhub_2021/", filename))
  cat("File", sQuote(filename), "saved for 2021 \n")
  
}

filename <- paste0("Labware_samples_2022_", file_date, ".rds")
saveRDS(labware_2022, paste0("Files_to_Jupyterhub_2022/", filename))
cat("File", sQuote(filename), "saved for 2022 \n")
