...
# Whether the data shall be saved (as Rdata and csv)  
# If the script has been run before on the same date, it will be overwritten
save_files <- TRUE
...
pars <- c("ALAD", "Aldrin", "BDE49", "Benzo[a]pyren", "EROD", "Fettinnhold", 
  ...
  "SCCP eksl. LOQ", "Toksafen Parlar 26", "Tørrstoff %", "Tributyltinn (TBT)", "VDSI")
...
# save_files is set to TRUE/FALSE in the beginnig of the script  
if (save_files){
...
## APPENDIX 3: Get chlorparaffine data (SCCP and MCCP)  
...
pars <- c(
  "SCCP eksl. LOQ", "SCCP inkl. LOQ",
  "MCCP eksl. LOQ", "MCCP inkl. LOQ")
...
pars <- c("SCCP eksl. LOQ", "SCCP inkl. LOQ")
#  "MCCP eksl. LOQ", "MCCP inkl. LOQ"
...
df_chemdata %>%
  ...
  # filter(n >= 15) %>%
  filter(STATION_CODE %in% c("02B", "19B", "20B", "23B", "30B", "898B1")) %>%
  ggplot(aes(VALUE)) +
  geom_histogram() +
  facet_grid(vars(NAME), vars(STATION_CODE))
...
test %>% filter(grepl("CCP", ANALYS)) %>% xtabs(~ANALYS, .) 
test %>% filter(grepl("CCP", REPORTED_NAME)) %>% xtabs(~REPORTED_NAME, .) 

names(test) %>% paste(collapse = ", ")
...
varstring <- "STATUS, STATUSX, PROSJEKT, TEXT_ID, SAMPLED_DATE, DESCRIPTION, AQUAMONITOR_ID, AQUAMONITOR_CODE, AQUAMONITOR_NAME, REPORTED_NAME, FORMATTED_ENTRY, NUMERIC_ENTRY, UNITS"

df_cp <- get_nivabase_selection(
  varstring,
  "LABWARE_IMPORT",
  "REPORTED_NAME",
  pars, values_are_text = TRUE, extra_sql = "and PROSJEKT = 'O 210200.ANAIN'")

# Alternative:
# df_cp <- get_nivabase_data(paste(
#   "select STATUS, STATUSX, PROSJEKT, TEXT_ID, SAMPLED_DATE, DESCRIPTION, AQUAMONITOR_ID, AQUAMONITOR_CODE,",
#   "AQUAMONITOR_NAME, REPORTED_NAME, FORMATTED_ENTRY, NUMERIC_ENTRY, UNITS",
#   "from NIVADATABASE.LABWARE_IMPORT where PROSJEKT = 'O 210200.ANAIN'",
#   "and REPORTED_NAME in ('SCCP eksl. LOQ', 'SCCP inkl. LOQ', 'MCCP eksl. LOQ', 'MCCP inkl. LOQ')"))

df_cp_wide <- df_cp %>%
  mutate(Year = year(SAMPLED_DATE)) %>%
  select(TEXT_ID, REPORTED_NAME, Year, FORMATTED_ENTRY) %>%
  # mutate(REPORTED_NAME = factor(REPORTED_NAME, levels = pars)) %>%
  tidyr::pivot_wider(names_from = REPORTED_NAME, values_from = FORMATTED_ENTRY, names_sort = TRUE)

# Confirm thar the inkl. LOQ vaøues never are ND (i.e. they are always a number and therefore imported
#   to Nivabase)
for (par in pars){
  cat("\n----------------\n", par, " equals 'ND':\n")
  table(df_cp_wide[[par]] == "ND") %>% print
}
