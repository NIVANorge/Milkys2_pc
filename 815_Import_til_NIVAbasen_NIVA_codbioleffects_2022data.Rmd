...
dat_cod <- readxl::read_excel("Input_files_2020/BEM-JAMP_2020_HEJ.xlsx", sheet = "Til Basen")

data_for_input_1a <- dat_cod %>%
  mutate(STATION_CODE = paste0(stasjon, "B")) %>%
  rename(Sample_no = nummer) %>%
  select(STATION_CODE, Sample_no, 
         `ALA-D (ng PBG/min mg protein)`, `Blodprotein (mg/mL)`, `EROD (pmol/min mg protein)`) %>%
...
# Original run in 2021, before SPECIES_ID was removed
...
# If run in 2022 or later, use this:
if (FALSE){
  df1 <- get_nivabase_selection(
