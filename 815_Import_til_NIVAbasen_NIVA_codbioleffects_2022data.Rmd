...
data_all <- readRDS(file = "Files_to_Jupyterhub_2022/01_df_2022_notstandard_2023-06-22.rds")
...
data_for_input_1a <- dat_cod %>%
  ...
  tidyr::pivot_longer(c(-STATION_CODE, -Sample_no), 
                      names_to = "Param", values_to = "Value") %>%  # note: no less-thans, so thos works  
  mutate(
    PARAM = case_when(
...
if (TRUE){
  ...
  if (nrow(check) > 0)
    stop("Some methods (METHOD_ID) are lacking")
...
- Lacking station 53B in Nivabasen  

```{r}

# Get Sørfjorden (53B) 2022 data  
