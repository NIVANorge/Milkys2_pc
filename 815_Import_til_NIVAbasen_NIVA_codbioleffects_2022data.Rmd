---
title: "815 - Import cod biological effect data from NILU to Nivabasen, 2020"
author: "DHJ"
date: "2 9 2019"
output: 
  html_document:
...
- *Cod bile metabolites*
    - until 2019, I also added these manually (through an R script like this)   
    - from 2020 onwards: non-normalized versions were entered through Labware (but with different names, see below)  
    - normalized versions and absorption are *not* entered through Labware 
- 2022 data:  
    - Run script in two bouts: first, for ALAD and EROD (adding samples), then for normalized PAH met and absorption (using existing samples added by the Labware to Nivabase import routine). **See 2a.4!**  
...
- absorption itself = AY380, ABS380 or AY (the two are the same)    
...
* If needed, use 'set_credentials' to store your username and password to R (only once per R session)  
* A better and more permanent solution is to use the keyring package to store the password securely on your PC  
...
dir("Files_to_Jupyterhub_2022", pattern = "01")

# Data at sample level (chemical data, biological efect parameters and VDSI)
data_all <- readRDS(file = "Files_to_Jupyterhub_2022/01_df_2022_notstandard_2023-08-09.rds")
...
sql <- paste(
  ...
  "and UPPER(PROSJEKT) = 'O 210200.ANAIN';")
sql
...
1. Biological effects in cod - from specimens that ARE in the NIVAbase, with the 
...
### a1 extra: Check gall metabolites of PAH    

```{r}

# No gall registered
cat("All existing tissues: \n")
table(addNA(df_samples$TISSUE))
cat("\n\n")

# Check all gall samples - SAMPLED_DATE is the only column missing 
sql <- paste(
  "select * from NIVADATABASE.LABWARE_CHECK_SAMPLE", 
  "where DESCRIPTION like '%galle%' AND",
  "TEXT_ID like '%2022%';")
# sql
df_labware_galle <- get_nivabase_data(sql)  

# Only 2021 dates found (!)  
cat("Dates for gall samples with 2022 in TEXT_ID: \n")
table(addNA(df_labware_galle$SAMPLED_DATE))
cat("\n")

# The dates that should be there  
station_date <- df_samples %>%
  filter(AQUAMONITOR_CODE %in% c("15B", "23B", "30B", "53B")) %>%
  distinct(AQUAMONITOR_CODE, SAMPLED_DATE)

writexl::write_xlsx(
  list(
    labware_galle = df_labware_galle,
    station_date = station_date
  ), 
  path = "Data/815_LIMS_PAH_metabolites_2022.xlsx")


if (FALSE){
  
  # Get data from Labware
  # Both 2021 and 2022
  labware_import_project <- get_nivabase_selection(
    "*",
    "LABWARE_IMPORT",
    "PROSJEKT",
    "O 210200.ANAIN", values_are_text = TRUE, extra_sql = " and AQUAMONITOR_CODE in ('15B', '23B', '30B', '53B')"
  )

  table(addNA(year(labware_import_project$SAMPLED_DATE)))
  
  # PAH metabolites in gall are there, but lack date
  # Therefore not being imported automatatically to Nivabase   
  labware_import_project %>%
    filter(grepl("OH", REPORTED_NAME)) %>%
    xtabs(~addNA(year(SAMPLED_DATE)) + REPORTED_NAME, .)  

  # COMPARE WITH EXCEL (sheet 'galle-omregning' to check that these are the un-normalised values   
  # (they are)
   #   2019     ~ 2020
   # "PA1OH"  ~ "1-OH-fenantren", 
   # "PYR1OH" ~ "1-OH-pyren", 
   # "BAP3OH" ~ "3-OH-benzo[a]pyren"
  no <- paste0("NR-2022-0", 9333:9347)
  labware_import_project %>%
    filter(TEXT_ID %in% no & REPORTED_NAME == "1-OH-pyren") %>%
    arrange(TEXT_ID) %>%
    select(TEXT_ID, DESCRIPTION, FORMATTED_ENTRY)
...
dat_cod <- readxl::read_excel("Input_files_2022/BEM-JAMP_2022_ARU.xls", sheet = "Til Basen")

# We could have used the first sheet if we only needed ALA-D and EROD  
# But we also need PROTV and ABS380
data_for_input_1a <- dat_cod %>%
  mutate(STATION_CODE = paste0(stasjon, "B")) %>%
  rename(Sample_no = nummer) %>% #    names()
  select(STATION_CODE, Sample_no, 
         `abs 380 nm`,
         `ala-d (ng PBG/min/mg prot)`, `EROD (pmol/min mg protein)`, `mikrosomalt protein (mg/mL)`, `blod protein (mg/mL)`) %>%
  tidyr::pivot_longer(c(-STATION_CODE, -Sample_no), 
                      names_to = "Param", values_to = "Value") %>%  # note: no less-thans, so this works  
  mutate(
    PARAM = case_when(
      grepl("EROD", Param, fixed = TRUE) ~ "EROD",
      grepl("mikrosomalt protein", Param, fixed = TRUE) ~ "PROTV",
      grepl("blod protein", Param, fixed = TRUE) ~ "PROTV",
      grepl("ala-d", Param, fixed = TRUE) ~ "ALAD",
      grepl("abs 380", Param, fixed = TRUE) ~ "AY380"
      ),
    METHOD_ID = case_when(
      grepl("EROD", Param, fixed = TRUE) ~ 28580,
      grepl("mikrosomalt protein", Param, fixed = TRUE) ~ 28575,
      grepl("blod protein", Param, fixed = TRUE) ~ 28575,
      grepl("ala-d", Param, fixed = TRUE) ~ 28579,
      grepl("abs 380", Param, fixed = TRUE) ~ 28574
      ),
    TISSUE_NAME = case_when(
      grepl("EROD", Param, fixed = TRUE) ~ "Liver - microsome",
      grepl("mikrosomalt protein", Param, fixed = TRUE) ~ "Liver - microsome",
      grepl("blod protein", Param, fixed = TRUE) ~ "Blod",
      grepl("ala-d", Param, fixed = TRUE) ~ "Blod",
      grepl("abs 380", Param, fixed = TRUE) ~ "Galle"
      ),
    LATIN_NAME = "Gadus morhua",
    Flag1 = as.character(NA),     # HARD_CODED - for these parameters, all values are above LOQ
    SPECIMEN_NO = Sample_no
    )
...
- NOT NEEDED (this is just "guidance" for a2)    

### a4. DELETE DATA NOT TO ADD NOW   

* In 2022, we want to add ALA-D, EROD, PROTV and AY380  
* Per 10.8.2023, we are still waiting for PAH metabolites to be transfered from LIMS (labware) til Nivadatabase  
* Therefore  
    - we will first add ALA-D, EROD, PROTV, as the samples needs to be added manually (blood and liver microsome)  
    - we will later (after PAH metabolites have been transfered, which also creates samples) add AY380  
     
```{r}

cat("Before: \n")
xtabs(~PARAM + TISSUE_NAME, data_for_input_1)

#
# For including only ALAD, EROD and PROTV  
# - must add records to BIOTA SAMPLES and BIOTA_SAMPLES_SPECIMENS further down
#
# data_for_input_1 <- data_for_input_1 %>%
#   filter(PARAM %in% c("ALAD", "EROD", "PROTV"))

#
# For including only AY380 (absorbance)    
# - do this after thre automatic Labware -> Nivabasen import has added records to BIOTA SAMPLES and BIOTA_SAMPLES_SPECIMENS
#
# data_for_input_1 <- data_for_input_1 %>%
#   filter(PARAM %in% c("AY380"))
...
df_species <- get_nivabase_selection("NIVA_TAXON_ID, LATIN_NAME", 
                                     "TAXONOMY", 
...
if (nrow(data_for_input_2) > nrow(data_for_input_1)){
  stop("There must be more than one sampling date per station! Check 'df_samples'.")
} else {
  message("Date added")
}
...
table(
  addNA(data_for_input_2$PARAM),
  addNA(data_for_input_2$METHOD_ID)
)
...
if (FALSE){
  ...
  df_absorp <- get_nivabase_selection(
    "*", 
    "METHOD_DEFINITIONS", 
    "NAME", 
    c("ABS380", "AY380", "AY"), values_are_text = TRUE)
  
  df_protv <- get_nivabase_selection(
    "*", 
    "METHOD_DEFINITIONS", 
    "NAME", 
    c("PROTV"), values_are_text = TRUE)
...
if ("METHOD_ID" %in% names(data_for_input_2)){
  
  df_lookup_methods <- get_nivabase_selection(
    "NAME, METHOD_ID, UNIT", 
    "METHOD_DEFINITIONS", 
    "METHOD_ID", 
    c(28580, 28575, 28579, 28574)     # ALAD, EROD, PROTV, AY380 
    )  
  data_for_input <- data_for_input_2 %>%
    left_join(df_lookup_methods, by = "METHOD_ID")  
  
  check <- data_for_input %>%
    filter(is.na(NAME)) %>%
    count(NAME, UNIT)
  
  if (nrow(check) > 0)
    stop("Some methods (METHOD_ID) are lacking or wrong")
...
- In our case:  
    - important for AY380 (absorption in the measuremnent of gall metabolites), 
    - not important where we add new tissues (blood and liver microsome)  
- NOTE for 2022 (27.6.2023): Lacking station 53B  

```{r}

yrs <- selected_year:(selected_year+1)
yrs_txt <- paste(yrs, collapse = ",")
...
existing_samples <- get_nivabase_selection(
  ...
  extra_sql = paste("and extract (YEAR from SAMPLE_DATE) in (", yrs_txt, ")") 
) %>%
...
xtabs(~STATION_CODE + TISSUE_NAME, existing_samples)
...
# In case some station (here, Sørfjorden 53B) is lacking  

if (FALSE){
  
  test <- get_nivabase_data(
    "select * from NIVADATABASE.LABWARE_IMPORT where PROSJEKT = 'O 210200.ANAIN' and DESCRIPTION like '53B%' and extract (YEAR from SAMPLED_DATE) = 2022")
  
  xtabs(~DESCRIPTION + STATUSX, test)  
  
  description_examples <- paste("53B Inner Sørfjord - Torsk galle", 1:2)
  xtabs(~DESCRIPTION + STATUSX, test %>% filter(DESCRIPTION %in% description_examples))  
  
  test %>% filter(DESCRIPTION %in% description_examples) %>% View()
...
biota_samples <- data_for_input %>%
  ...
  left_join(df_tissue,                                                      # adds TISSUE_ID
            na_matches = "never", by = "TISSUE_NAME")  %>%
  mutate(
...
biota_samples_fromdatabase <- niRvana::get_nivabase_selection(
  ...
  extra_sql = paste(
    "and TRUNC(ENTERED_DATE) = TO_DATE('2023-08-10', 'yyyy-mm-dd')",    # HARD-CODED
    "and ENTERED_BY = 'DHJ'")
)
...
biota_samples_temp <- biota_samples %>%
  left_join(biota_samples_fromdatabase %>% select(STATION_ID, TISSUE_ID, TAXONOMY_CODE_ID, SAMPLE_NO, SAMPLE_ID), 
                 by = c("STATION_ID", "TISSUE_ID", "TAXONOMY_CODE_ID", "SAMPLE_NO"),
                 na_matches = "never") %>%
  left_join(df_stations %>% select(STATION_ID, STATION_CODE),   # add STATION_CODE as well  
                 by = "STATION_ID",
                 na_matches = "never")

if (nrow(biota_samples_temp) != nrow(biota_samples)){
  stop("Check joins! Number of rows changed")
}  else  {
  biota_samples <- biota_samples_temp
}

cat("Number of records: ")
length(unique(biota_samples_temp$SAMPLE_ID)) %>% cat()
...
existing_singlespec_1 <- get_nivabase_selection(
  ...
  "STATION_ID",
  unique(data_for_input$STATION_ID),
  extra_sql = " and extract(YEAR from DATE_CAUGHT) = 2020"
)
# Check
...
sql <- paste(
  ...
  "where extract(YEAR from SAMPLED_DATE) =", selected_year, 
  "and extract(MONTH from SAMPLED_DATE) >= 3", 
  "and PROSJEKT like '%O 210200.ANAIN%'")       # HARD-CODED
df_labware_01 <- get_nivabase_data(sql)   
...
df_samples %>%
  ...
  mutate(Pooled_sample = !is.na(X_BULK_BIO)) %>%
  ggplot(aes(AQUAMONITOR_CODE, BIOTA_SAMPLENO, color = Pooled_sample)) +   # changed from Specimen_no
  geom_point() +
...
biota_specimens_to_add <- biota_specimens_to_add %>%
  left_join(existing_singlespec_date %>% select(-n), 
                 na_matches = "never",
                 by = "STATION_ID")
...
- NOT NEEDED if zero specimen records will be added - as this gets all specimens just added, and we didn't need to add any 
...
if (TRUE){
  ...
  biota_specimens_fromdatabase <- get_nivabase_selection(
    ...
    "STATION_ID",
    unique(data_for_input$STATION_ID),
    extra_sql = paste(" and extract(YEAR from DATE_CAUGHT) =", selected_year)
    )
...
biota_chemistry_values %>%
  count(STATION_ID, SAMPLE_NO, METHOD_ID, TISSUE_ID) %>%
...
biota_chemistry_values %>%
  xtabs(~SAMPLE_ID + paste(METHOD_ID, TISSUE_ID), .)
...
### e. EXTRA - Delete PROTV for 'Liver microsome'  

```{r}

# AFTER running code of 802

df_error <- df_chemdata %>%
  filter(NAME == "PROTV")
# View(df_error)
nrow(df_error)  # 46

xtabs(~STATION_CODE + year(SAMPLE_DATE), df_error)
xtabs(~METHOD_ID, df_error)   # 28575 

# SELECT COUNT(*) from NIVADATABASE.BIOTA_CHEMISTRY_VALUES where TRUNC(ENTERED_DATE) = TO_DATE('2023-08-10', 'yyyy-mm-dd') and ENTERED_BY = 'DHJ' and METHOD_ID = 28575;

# DELETE from NIVADATABASE.BIOTA_CHEMISTRY_VALUES where TRUNC(ENTERED_DATE) = TO_DATE('2023-08-10', 'yyyy-mm-dd') and ENTERED_BY = 'DHJ' and METHOD_ID = 28575;
# commit;
...
get_specimens_from_stationdata <- function (station_data, years = NULL) {
  if (!is.null(years)){
    extra_sql <- paste0(" and extract (YEAR from DATE_CAUGHT) in (", 
...
df_specimens_allyrs <- get_nivabase_selection(
  ...
  "STATION_ID", 
  unique(data_for_input$STATION_ID),
  extra_sql = " and extract (YEAR from DATE_CAUGHT) > 2014")
...
# All gall parameters  
xtabs(~PARAM + MYEAR, dat_compl %>% filter(TISSUE_NAME == "Galle"))

# Absorpion
xtabs(~PARAM + MYEAR, dat_compl %>% filter(PARAM %in% c("ABS380","AY","AY380")))
...
gg <- dat_compl %>% 
  filter(PARAM %in% c("PYR1O", "PYR1OH")) %>%
  ggplot() +
  geom_point(aes(x = MYEAR, y = VALUE_WW, shape = is.na(FLAG1), color = PARAM)) +
