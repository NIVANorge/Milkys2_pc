...
# data <- read_ices_file(fn, sep = ";") # 2020

data <- add_field_codes(data)  
...
for (i in 1:9){
  for (j in 1:ncol(data[[i]])){
    if (names(data[[i]])[j] %in% vars_numeric)
...
cols <- var_overlap(data[["04"]], data[["10"]], drop = "RECID")
...
df1 <- data[["03"]]
df2 <- data[["04"]]
cols <- var_overlap(df1, df2, drop = "RECID")
