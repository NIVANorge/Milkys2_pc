...
save_folder_txt <- "Files_to_Jupyterhub_2020/842_ICES/Test"
save_folder_rds <- "Files_to_Jupyterhub_2020/842_ICES/Rdata"
...
if (sum(df_amlnk$PARAM == "INTF%") == 0){
  # add one row to df_amlnk
  df_amlnk <- bind_rows(
    df_amlnk,
    tibble(PARAM = "INTF%", AMLNK = max(df_amlnk$AMLNK) + 1)
  )
}
...
```
### c2. Add 71A (the actual Bodø harbour)     
```{r}
...
df_dates <- df_dates %>%
  group_by(STNNO) %>%
  summarise(SAMPLE_DATE = mean(SAMPLE_DATE) %>% floor_date("day"),
            .groups = "drop") %>%
  mutate(
...
#
# 2019: We lacked date for 26A2 - used code in brackets to find it  
#
if (FALSE){
...
if (write_ices_file){
  ...
  fns <- dir(save_folder_txt, paste0("NIVA", current_year, "CF*"))
  ...
  string <- paste0("NIVA", current_year, "CF_%02i.NO")
...
if (write_ices_file){
  ...
  string <- paste0("842_NIVA", current_year, "CF_%02i.rds")
...
if (FALSE){
  ...
  library(writexl)
  string <- paste0("842_NIVA", current_year, "CF_%02i.xlsx")
  fn_xl <- sprintf(string, version)
  
  write_xlsx(datalist, paste0("Data/", fn_xl))
