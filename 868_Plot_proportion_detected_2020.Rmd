...
* Years/parameters where >50% are under LOQ are shown with smaller letters   
* Fill colours follows percentiles 0,10,20,...100  
* How to get white text on the darker colours: see 9

```{r, fig.width=11, fig.height=16}

# Split by percentiles (deciles) for colors  
...
## 9. Median values, test single parameter in many stations     

* Also making white text on the darker colours
...
# Get N_less_thans per year
# - in order to separate between years with no data, and years with no less-thans   
df_medians_fish_param <- dat %>%
  filter(MYEAR %in% 2000:2020 &
           PARAM %in% param &                        # as 8, just change here
           TISSUE_NAME == "Lever" &   
           STATION_CODE %in% stations_fish) %>%
  group_by(STATION_CODE, MYEAR) %>%                  # and here  
  summarise(
    N = length(FLAG1),
    N_less_thans = sum(!is.na(FLAG1)),
    VALUE_WW = median(VALUE_WW), 
...
brs <- quantile(df_medians_fish_param$log_VALUE_WW, seq(0,1,0.1), na.rm = TRUE)

# tissues_plot <- c("Whole soft body", "Lever", "Muskel", "Blod", "Egg")  # Muskel is not so interesting
tissues_plot <- c("Whole soft body", "Lever", "Blod", "Egg")

ggplot(df_medians_fish_param %>%
         # filter(PARAM %in% param_levels) %>%
         mutate(
           # PARAM = factor(PARAM, levels = rev(param_levels)), # rev: ordering top -> down
           STATION_CODE = factor(STATION_CODE, levels = stations_fish),
           ...
           Over_LOQ = case_when(
             ...
             N_less_thans >= 0.5*N ~ "Under LOQ"),
           Textcolor = case_when(
             log_VALUE_WW %in% levels(log_VALUE_WW)[c(1,2,9,10)] ~ "white",
             TRUE ~ "black")
         ), #%>%
       # filter(PARAM != "NAP"),
       aes(Year, STATION_CODE, fill = log_VALUE_WW)) +
  geom_raster() +
  scale_fill_brewer(palette = "RdYlBu", direction = -1) +
  geom_text(aes(label = VALUE_WW, size = Over_LOQ, color = Textcolor)) +
  scale_size_manual(values = 3:2) +
  scale_color_manual(values = c(black = "black", white = "white")) +
  theme(axis.text.x = element_text(hjust = 0, angle = -30)) +
  theme(axis.text.y = element_text(size = 7)) +
  guides(color = "none")   # remove legend for black/white text


```


### Same, with uniform scale on the log scale        
```{r, fig.width=9.5, fig.height=5}

brs <- quantile(df_medians_fish_param$log_VALUE_WW, seq(0,1,0.1), na.rm = TRUE)

brs <- seq(min(df_medians_fish_param$log_VALUE_WW, na.rm = TRUE),
           max(df_medians_fish_param$log_VALUE_WW, na.rm = TRUE),
           length = 11)
...
### Same, with uniform scale on the non-log scale        
```{r, fig.width=9.5, fig.height=5}

brs <- quantile(df_medians_fish_param$log_VALUE_WW, seq(0,1,0.1), na.rm = TRUE)

brs <- seq(min(df_medians_fish_param$VALUE_WW, na.rm = TRUE),
           max(df_medians_fish_param$VALUE_WW, na.rm = TRUE),
           length = 11)
...
ggplot(df_medians_fish_param %>%
         ...
         mutate(
           ...
           Over_LOQ = case_when(
             ...
             N_less_thans >= 0.5*N ~ "Under LOQ"),
           Textcolor = case_when(
             VALUE_WW_cut %in% levels(VALUE_WW_cut)[c(1,2,9,10)] ~ "white",
             TRUE ~ "black")
         ), #%>%
       # filter(PARAM != "NAP"),
       aes(Year, STATION_CODE, fill = VALUE_WW_cut)) +
  geom_raster() +
  scale_fill_brewer(palette = "RdYlBu", direction = -1) +
  geom_text(aes(label = VALUE_WW, size = Over_LOQ, color = Textcolor)) +
  scale_size_manual(values = 3:2) +
