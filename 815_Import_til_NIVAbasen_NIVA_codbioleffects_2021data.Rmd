...
if (FALSE){
  df_samples %>% 
    arrange(AQUAMONITOR_CODE, Specimen_no) %>% 
    select(ANALYSEOPPDRAG, TEXT_ID, SAMPLED_DATE, STATUS, DESCRIPTION) %>% 
    View()
}

check <- df_sampledates %>%
  add_count(AQUAMONITOR_CODE) %>%
  filter(n > 1)

if (nrow(check) > 0){
  stop("There seems to be more than one sampling date per station!")
} else {
  message("Date added")
}
