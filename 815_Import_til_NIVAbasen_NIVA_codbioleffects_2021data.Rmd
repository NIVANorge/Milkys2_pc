...
- *Cod bile metabolites*: here 2020 data differs from older data: until 2019, I also added these manually (through an R script like this), in 2020-2021, they were entered through Labware (but with different names, see below)      
- cod bile metabolite names until 2019, and in 2020-2021:   
```
   2019     ~ 2020
...
existing_samples <- get_nivabase_selection(
  ...
  extra_sql = paste("and extract (YEAR from SAMPLE_DATE) =", selected_year)
) %>%
...
Makes `biota_samples1`, used in 5  
...
# "SPECIES_ID"           - Has been excluded from table (was usually NA anyway)
...
xtabs(~addNA(STATION_ID) + addNA(TISSUE_NAME), biota_samples1)
...
### b. Check 'biota_samples1' vs single specimens   
NOT NEEDED? Done in 10a-b.  
  
Check whether all stations/samples in 'biota_samples1' has a corresponding fish in BIOTA_SINGLE_SPECIMENS  
...
sql_list <- 1:nrow(biota_samples1) %>% 
  map_chr(make_sql_sample, data = biota_samples1)
...
day_for_adding_records <- "2022-08-08"

biota_samples_fromdatabase <- niRvana::get_nivabase_selection(
  "*",
  "BIOTA_SAMPLES",
  "STATION_ID",
  biota_samples1$STATION_ID, 
  extra_sql = paste0(
...
# DELETE from NIVADATABASE.BIOTA_SAMPLES where TRUNC(ENTERED_DATE) = TO_DATE('2022-08-08', 'yyyy-mm-dd') and ENTERED_BY = 'DHJ';
# commit;
...
biota_samples2 <- biota_samples1 %>%
  safe_left_join(biota_samples_fromdatabase %>% 
                   select(STATION_ID, TISSUE_ID, TAXONOMY_CODE_ID, SAMPLE_NO, SAMPLE_ID), 
...
n_records3 <- length(unique(biota_samples2$SAMPLE_ID))
cat("Number of records: ", n_records3, "\n\n")

if (n_records3 != n_records1){
  stop("Number of added records is not the same as number of SQLs in the 'Make SQLs' section!")
} else {
  cat("The number of added records is the same as number of SQLs in the 'Make SQLs' section \n")
}
...
# Check ID
cat("SAMPLE_ID: ")
range(biota_samples2$SAMPLE_ID) %>% paste(collapse = " - ") %>% cat()
cat("\n")

```

### h. Visualise `biota_samples2`     
```{r}
...
if (FALSE) {
  ...
  tab <- xtabs(~SAMPLE_ID, biota_samples2)
  table(tab)
  
  # Check that SAMPLE_NO + TISSUE_NAME + STATION_ID are unique
  xtabs(~SAMPLE_NO + TISSUE_NAME + STATION_ID, biota_samples2)
  
}


ggplot(biota_samples2, aes(TISSUE_NAME, SAMPLE_NO)) +
  geom_point() +
...
sql <- paste(
  ...
  "where extract(YEAR from SAMPLED_DATE) =", selected_year, 
  "and extract(MONTH from SAMPLED_DATE) >= 3", 
  "and UPPER(PROSJEKT) like '%210200%'")       # '210200' is HARD-CODED
df_labware_01 <- get_nivabase_data(sql)   
nrow(df_labware_01)
...
df_samples <- bind_rows(df_labware_01, df_labware_02) %>%
  ...
  mutate(
    ...
    Specimen_no = stringr::str_extract(DESCRIPTION2, "[0-9]+") %>% as.numeric(),
    TISSUE_NAME = stringr::str_sub(TISSUE, start = 4),
    SIA = grepl("SIA", DESCRIPTION),
    # New variable 'Tissue': equals TISSUE_NAME, but separates 'Muskel' and 'Muskel SIA' 
    Tissue = case_when(
      !SIA ~ TISSUE_NAME,
      SIA ~ paste(TISSUE_NAME, "SIA")),
    Sample_type = case_when(
      is.na(X_BULK_BIO) ~ "Sample no = ind no",
      as.character(BIOTA_SAMPLENO) == X_BULK_BIO ~ "Sample no = ind no",
      grepl(",", X_BULK_BIO, fixed = TRUE) ~ "Pooled sample",
      TRUE ~ "Sample no != ind no")
  )  # SIA samples: samples taken for isotope analysis
...
- Yes, there are now also bile (but not blood and liver-microsome, so no need to change anything)     
- Also shows whether samples are pooled or whether they have different specimen and sample numbers   
```{r, fig.width=7, fig.height=7}

# Note: code and figure also utilised in app 807

df_samples %>%
  filter(AQUAMONITOR_CODE %in% data_for_input$STATION_CODE) %>%
  arrange(BIOTA_SAMPLENO) %>%
  mutate(BIOTA_SAMPLENO = factor(BIOTA_SAMPLENO) %>% fct_inorder()) %>%
  ggplot(aes(AQUAMONITOR_CODE, BIOTA_SAMPLENO, color = Sample_type)) + 
  geom_point() +
  geom_text(aes(label = X_BULK_BIO), nudge_x = 0.1, hjust = 0, size = 3) +
  facet_grid(vars(Tissue))


```

### b3. Add SPECIMEN_NO to biota_samples2     
...
biota_specimens_to_add <- biota_samples3 %>%
  anti_join(existing_singlespec_1, by = c("STATION_ID", "SPECIMEN_NO")) %>%
...
- CASE B: All specimens for given stations  
Directly from R, instead of pasting into SQL Developer for getting data  
- in contrast to the original of this code (for 2018 data)   
Remember to **commit** in SQL developer first   
...
Add records for   
1. The new specimens  
2. The 'old' specimens, linking to new samples (biota_samples3)  
...
biota_specimens_complete <- niRvana::get_nivabase_selection(
  ...
  "BIOTA_SINGLE_SPECIMENS",
  "STATION_ID",
  unique(biota_samples3$STATION_ID), 
  extra_sql = paste(
    " and extract(YEAR from DATE_CAUGHT) =",
    selected_year)
    )
...
# biota_samples3 already have SPECIMEN_NO added (from 10b)
# biota_samples_specimens <- biota_samples3 %>%
...
n_records4 <- nrow(biota_samples_specimens)  # 92
cat("Number of records that will be added to BIOTA_SAMPLES_SPECIMENS: ", n_records4, "\n\n")  

if (n_records4 != n_records1){
  stop("Number of records to add is not the same as number of SQLs in the 'Make SQLs' section! (In case of pooled samples, this is not an error.)")
} else {
  cat("The number of added records is the same as number of SQLs in the 'Make SQLs' section \n")
}
...
# "SAMPLE_ID"            - Get from 'biota_samples3'
...
cat("===================================\n")
cat("Biota samples that has been inserted in database: \n")
xtabs(~addNA(STATION_ID) + addNA(TISSUE_NAME), biota_samples3)
...
dat_test %>%
  ...
  scale_x_continuous(breaks = 2014:selected_year) +
  theme(axis.text.x = element_text(angle = -45, hjust = 0))
...
df_specimens <- get_specimens_from_stationdata(df_stations, years = selected_year)
...
df_selected_year <- get_biota_chemistry(
  years = selected_year + seq(0,1),              # in case some specimens wrongly  
                                                 #    have date_caught = later yaers
  specimendata = df_specimens, 
  stationdata = df_stations,
  report_samples = TRUE)

df_selected_year <- df_selected_year %>% rename(PARAM = NAME)
...
df_selected_year_new <- get_biota_chemistry(
  years = selected_year + 0:1, 
  specimendata = df_specimens, 
  stationdata = df_stations,
  report_samples = TRUE) %>%
  rename(PARAM = NAME)

df_selected_year_new %>%
  filter(PARAM %in% params) %>%
  xtabs(~PARAM + STATION_CODE, .)  
...
df0 <- get_nivabase_selection(
  "STATION_ID, STATION_CODE, STATION_NAME",
  "PROJECTS_STATIONS",
  "STATION_CODE",
  "71B", values_are_text = TRUE)
# df0$STATION_ID

df1 <- get_nivabase_selection(
  "STATION_ID, SPECIMEN_ID, SPECIMEN_NO, DATE_CAUGHT",
  "BIOTA_SINGLE_SPECIMENS",
  "STATION_ID",
  df0$STATION_ID[1],
  extra_sql = paste(" and extract(YEAR from DATE_CAUGHT) =", selected_year))
...
df3 <- get_nivabase_selection(
  "SAMPLE_ID, SAMPLE_NO, REPNO, TISSUE_ID",
  "BIOTA_SAMPLES",
  "SAMPLE_ID",
  df2$SAMPLE_ID)

df3_tissue <- get_nivabase_selection(
  "TISSUE_ID, TISSUE_NAME",
  "BIOTA_TISSUE_TYPES",
  "TISSUE_ID",
  unique(df3$TISSUE_ID))
...
df4_methods <- get_nivabase_selection(
  "METHOD_ID, NAME, UNIT, LABORATORY", 
  "METHOD_DEFINITIONS", 
  "METHOD_ID", 
  unique(df4$METHOD_ID))

df_check <- df3 %>%
  left_join(df3_tissue) %>%
  left_join(df4) %>%
  left_join(df4_methods) %>%
  left_join(df2) %>%
  left_join(df1) %>%
  left_join(df0)

# Siloxans - check values  
df_check %>%
  filter(SAMPLE_NO > 100) %>% # View()
  ggplot(aes(SAMPLE_NO, VALUE)) +
    geom_point(color = "blue3") +
    facet_grid(cols = vars(NAME))

# Siloxans - check individuals to samples  
df_check %>%
  group_by(SPECIMEN_NO, TISSUE_NAME, SAMPLE_NO) %>%
  
  arrange(SPECIMEN_NO, TISSUE_NAME, SAMPLE_NO)
  
df_check %>%
  group_by(SPECIMEN_ID, SPECIMEN_NO, TISSUE_NAME, SAMPLE_NO) %>%
  summarise(
    No_param = n(),
    Parameters = substr(paste(unique(NAME), collapse = ", "), 1, 25)
  ) %>%
  arrange(SPECIMEN_ID, SPECIMEN_NO, TISSUE_NAME, SAMPLE_NO)
