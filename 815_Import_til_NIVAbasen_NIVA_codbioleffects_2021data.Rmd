...
today <- substr(lubridate::now(tzone = "UTC"), 1, 10)
...
biota_samples_fromdatabase <- niRvana::get_nivabase_selection(
  ...
  extra_sql = paste0(
    "and TRUNC(ENTERED_DATE) = TO_DATE(", sQuote(today), ", 'yyyy-mm-dd')", 
    " and ENTERED_BY = 'DHJ'")
)

cat("Number of records: ")
nrow(biota_samples_fromdatabase) %>% cat()   # 144
cat("\nShould be the same as number of SQLs in the 'Make SQLs' section! \n")
