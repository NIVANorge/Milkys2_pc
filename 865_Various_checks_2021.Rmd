...
## 6. Check Elkem Carbon og REC Solar 2022 data 

### Get a list of the stations in the CEMP_Biota project

```{r}

df_stations <- get_stations_from_project("Elkem Carbon", ignore.case = FALSE)

df_stations <- get_stations_from_project(
  "Tiltaksorientert overvåking for Elkem Carbon og REC Solar i 2022", ignore.case = FALSE)
...
### Biota parameters and stations  
```{r}

xtabs(~NAME + STATION_CODE, dat_base)
...
df_projects %>%
  filter(PROJECT_ID == 12625)

df_stations <- get_stations_from_project("MILFERSK 2021-2025", ignore.case = FALSE)
...
df_nilu <- get_nivabase_selection(
  "*",
  "METHOD_DEFINITIONS",
  "LABORATORY",
