...
NOTE: If just some extra data (from existing samples) shall be added, nost of this can be skipped - see part 81!  

For coupling excel data of Labware data, we:

1. Create the sample table 'df_lookup_samples'  
    - see explanation of this file in the [README](README.html)!
2. Coupling existing samples in Nivabase to excel data in the following ways (see part 60):
    - using TEXT_ID for all parameters except siloxans ('dat_insert_a' in 60a)  
    - Using TISSUE_NAME + SPECIMEN_NO for siloxans ('dat_insert_b' in 60a)  
3. In addition: Two samples (blod 10 + 11) have siloxan data but lack in Labware    
   - These two samples MUST BE ADDED (see 5c and entire part 20)    
...
### c. Add/check SAMPLE_ID  

```{r}

if (FALSE){
  
  # this part is NOT NEEDED HERE  (did that in a1)
...
- Before the part below, run 1-4 above
- Below, 'dat_excel_1' is filtered to only contain data we want to add   
- Also, we create 'df_lookup_samples_new'  
- After this, run part 60 ('BIOTA_CHEMISTRY_VALUES')  
```{r}
...
## 82. Add HBCD data for eider duck blood   
- These were not in the excel file when I inserted the data  
- NOTE: Follow "manual" given above the chunk below     

### a. Filter 'dat_excel_1'  
- NOTE: Before the part below, run 1-4 above
- Below, 'dat_excel_1' is filtered to only contain data we want to add   
- Also, we create 'df_lookup_samples_new'  
- After this, run part 60 ('BIOTA_CHEMISTRY_VALUES')  
```{r}
...
if (FALSE){
  ...
  dat_excel_1 <- dat_excel_1 %>%
    filter(Group %in% "HBCD" & TISSUE_NAME %in% "Blod")
