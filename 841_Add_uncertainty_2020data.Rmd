...
### Compare QUANTIFICATION_LIMIT and LOQ from less-than   
    - `LOQ_lt_med` (median value of less-than's) will be used where QUANTIFICATION_LIMIT is lacking
...
mean_uncrt <- dat_all_updated %>%
  ...
  group_by(MYEAR, Lab, LATIN_NAME, TISSUE_NAME,<6 PARAM) %>%
  summarise(
...
plot_error <- function(lab, param, tissue){
  df_plot <- dat_all_updated %>%
    filter(MYEAR == selected_year & !is.na(UNCRT)) %>%
    filter(Lab == lab & PARAM == param & TISSUE_NAME == tissue) %>%
  ...
  gg <- ggplot(df_plot, aes(VALUE_WW, Percent_error)) +
    ...
    xlim(0,50) +
    labs(title = paste0(param, " (", lab, ") in ", tissue),
...
plot_error("Eurofins", "CB180", "Lever")
