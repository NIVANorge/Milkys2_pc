...
```
### Plot raw data   
- Discovered further down that especially some PFDcA values from Sørfjorden are very low  
- Also some PFUdA from and Bergen Havn Sørfjorden are very low  
```{r}
...
dat_all_allyears_view <- dat_all_allyears %>% 
  select(
    ...
    SAMPLE_NO2, VALUE_WW, FLAG1, Lab, QUANTIFICATION_LIMIT, UNCERTAINTY
  )
...
- to use where there are neither QUANTIFICATION_LIMIT or less-thans  
```{r}
...
### Overall minimum value llower than min. less-than  
```{r}
...
## 8. Using minimum less-than value as LOQ  
- I.e. LOQ_lt_min  
- Also assume that LOD = LOQ/1.33

### Add UNCRT and METCU  
We here use LOQ from less-thans as LOQ, if possible  
    - i.e. LOQ_lt_min, or QUANTIFICATION_LIMIT if LOQ_lt_min is not given  
    - we here assume that LOD = LOQ/1.333 (so s = 'LOQ_lt_min/4')
```{r}

dat_all_updated <- dat_all_updated %>%
  mutate(
    LOD = case_when(
      !is.na(LOQ_lt_min) ~ LOQ_lt_min/1.333,
      is.na(LOQ_lt_min) ~ QUANTIFICATION_LIMIT/1.333),
    s = LOD/3,
    UNCRT = case_when(
      !is.na(s) ~ sqrt(s^2 + (VALUE_WW^2)*((Uncertainty/100)^2)),
      is.na(s) ~ Uncertainty),   # should be zero cases of this
...
xtabs(~is.na(UNCRT), dat_all_updated %>% filter(MYEAR == selected_year))
cat("\n")
cat("Number of 's' lacking: \n")
xtabs(~is.na(s), dat_all_updated %>% filter(MYEAR == selected_year))


dat_all_updated %>% names() %>% paste(collapse = ", ")
...
dat_all_updated_view <- dat_all_updated %>% 
  select(
    STATION_CODE, STATION_NAME, SAMPLE_DATE, LATIN_NAME, TISSUE_NAME, PARAM, MYEAR, 
    SAMPLE_NO2, VALUE_WW, FLAG1, Lab, QUANTIFICATION_LIMIT, Uncertainty, 
    LOQ_lt_min, LOQ_lt_med, Min_value, LOQ_source, LOD, s, UNCRT, METCU
  )
...
plot_error("Eurofins", "CB180", "Gadus morhua", "Lever")
...
df <- dat_all_updated %>% 
  filter(MYEAR == selected_year & is.na(FLAG1) & 
           ...
           TISSUE_NAME == "Muskel")
df %>%
  select(VALUE_WW, Uncertainty, LOQ_lt_min, Min_value, UNCRT, METCU)
