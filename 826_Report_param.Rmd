...
Notes from meeting w/ Merete G + 
- Viktig: Ikke klassifiser EQS/proref når LOQ>EQS - gjør om til -1 i den store excel  
- Blått i nord, rødt i sør  (DONE i Jupyterhub)  
- Kanskje ikke Times font i interaktive figurer
- Tidsserier 10 år i faktaark, + alle år i elektronisk vedlegg 
- Sende fargekoder for Proref til Merete G (DONE)
...
fn <- "Files_from_Jupyterhub_2021/Raw_data/109_adjusted_data_2022-09-01.rds"          
dat_all_orig <- readRDS(fn)


fn <- "Files_from_Jupyterhub_2021/Raw_data/999_stations.xlsx"
readxl::excel_sheets(fn)
lookup_stations_cod <- read_excel(fn, "Torsk")
lookup_stations_bluemussel <- read_excel(fn, 2)

lookup_stations <- bind_rows(lookup_stations_cod, lookup_stations_bluemussel)
...
lookup_eqs <- read.csv("Files_from_Jupyterhub_2021/Raw_data/EQS_limits.csv") %>%
  select(-Long_name, -Kommentar)

lookup_proref <- read.csv("Files_from_Jupyterhub_2021/Raw_data/Proref_used.csv")

# For now, WW only
dat_2 <- dat_1 %>%
  left_join(
    lookup_eqs %>% filter(is.na(LATIN_NAME) & Basis == "WW") %>% select(-LATIN_NAME) %>% rename(EQS_WW = Limit), 
    by = c("PARAM")
    ) %>%
  left_join(
    lookup_proref %>% filter(Basis == "WW") %>% select(PARAM, LATIN_NAME, TISSUE_NAME, Q95) %>% rename(Proref = Q95), 
...
dat_2 <- dat_1 %>%
  left_join(
    lookup_eqs %>% filter(is.na(LATIN_NAME)) %>% select(-LATIN_NAME) %>% rename(Limit1 = Limit), 
    by = c("PARAM", "TISSUE_NAME")) %>%
  left_join(
    lookup_eqs %>% filter(!is.na(LATIN_NAME)) %>% rename(Limit2 = Limit), 
    by = c("PARAM", "LATIN_NAME", "TISSUE_NAME")) %>%
  mutate(
    Above_EQS = case_when(
...
station_order_fish <- with(lookup_stations_cod, paste(STATION_CODE, Station_name))
station_order_mussel <- with(lookup_stations_bluemussel, paste(STATION_CODE, Station_name))
...
dat_trend_all <- readRDS("Files_from_Jupyterhub_2021/Raw_data/126_df_trend_2021.rds")

dat_trend_1 <- dat_trend_all %>%
  filter(
    PARAM %in% param, 
    LATIN_NAME %in% species,
    TISSUE_NAME %in% tissue)
...
gg_trend_list <- c("long", "short") %>%
  map(
    ~ggplot(subset(dat_trend, Trend_type == .x),
            aes(x = Station, y = Perc_annual)) +
      geom_linerange(aes(ymin = Perc_annual_lo, ymax = Perc_annual_hi, color = Trend_string)) +
      geom_point(aes(fill = Trend_string, shape = Trend_string), color = "black") +
